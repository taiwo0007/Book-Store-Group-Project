...
def highRatedBooks(request):

    booksPopulateF = Book.objects.filter(category='b6e303f8-d681-4aa2-ab8a-ad3b27a62015')
    booksRatingFiction = booksPopulateF.order_by('-star_rating')[:6]

    booksPopC= Book.objects.filter(category='1a8c0bf6-85cf-4170-ab6b-45604dd43cf2')
    booksRatingChildren = booksPopC.order_by('-star_rating')[:6]

    booksPopN= Book.objects.filter(category='f1b46f0d-7ee6-4ecb-a649-aaeb6407f836')
    booksRatingNon = booksPopN.order_by('-star_rating')[:6]

    for i in booksRatingChildren:
        print(i.star_rating)

    for l in booksRatingFiction:
        print(l.star_rating)
    
    for j in booksRatingNon:
        print(j.star_rating)



    return render(request, 'shop/book_rating.html', {'booksRatingChildren':booksRatingChildren, 'booksRatingFiction':booksRatingFiction,'booksRatingNon':booksRatingNon })
